...
Чтобы его запустить нужно проделать следующие действия:
1. Убедиться что на компьютере установлен Docker Desktop
2. Клонировать репизиторий ```git clone https://github.com/vMaxiMv/Mus_lib.git```
   Перейти в корневую директорию проекта ```cd /Mus_lib ```
3. Создать .env файл со следующей переменной:
   ```DATABASE_URL="postgresql://postgres:ippdbdmbbmbu@localhost:5432/mus_library"```
