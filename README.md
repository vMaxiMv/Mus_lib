...
## Этап 4 Разработать программу для работы с вашей базой данных. 
Программа для работы с базой данной представляет собой веб приложение.
Для начала при помощи скриптов для создания и заполнения базы данных (см. Приложение) заполняем базу данных.
Для того чтобы соединить приложение с базой, создаем в папке /server файл .env и прописываем там DATABASE_URL="postgresql://postgres:{пароль_от_postgres}@localhost:5432/mus_lib"
Также для запуска приложения нужно иметь установленный на компьютере node js (моя версия 19.4.0)

После клонирования репозитория и установки всех необходимых зависимостей можно запустить приложение
Для запуска серверной части переходим в папку server и пишем там ```npm run dev```, сервер запуститься на 5000 порту
Для запуска клиентской части переходим в папку client и пишем там ```npm start```, клиент запуститься на 3000 порту
Приложение будет доступно по ссылке http://localhost:3000/ в браузере

### Скриншоты из приложениея
Список всех музыкальных произведений из базы данных
![](GUI_screeshots/tracks_list.jpg)

Форма для добавления нового музыкального произведения в базу данных
![](GUI_screeshots/track_create_form.jpg)

Страница с выбранным музыкальным произведением с информацией, взятой из связных таблиц об этом музыкальном произведении и кнопки для удаления и редактирования этого трека
![](GUI_screeshots/track_detail.jpg)
